      ...
      <template v-else>
        ...
        <div class="report-page__stats">
          <div class="report-page__stat card">
            <span class="report-page__stat-label">{{ $t('report.totalElapsed') }}</span>
            <span class="report-page__stat-value">{{ formatDuration(displayTotalElapsed) }}</span>
          </div>
          <div v-if="displayPlannedDuration > 0" class="report-page__stat card">
            <span class="report-page__stat-label">{{ $t('report.plannedDuration') }}</span>
            <span class="report-page__stat-value">{{ formatDuration(displayPlannedDuration) }}</span>
          </div>
          <div class="report-page__stat card">
...
import { getImprovementText } from '@/utils/reportPresentation'

Chart.register(...registerables)

const REPORT_POLL_ATTEMPTS = 5
const REPORT_POLL_DELAY_MS = 400
...
const loadBackendReport = async () => {
  for (let attempt = 0; attempt < REPORT_POLL_ATTEMPTS; attempt++) {
    ...
    if (attempt < REPORT_POLL_ATTEMPTS - 1) {
...
const displayTotalElapsed = computed(() => {
  if (typeof backendReport.value?.duration_seconds === 'number') {
    return backendReport.value.duration_seconds
  }
  if (backendReport.value?.duration_minutes) {
    return Math.round(backendReport.value.duration_minutes * 60)
  }
...
const displayPlannedDuration = computed(() => {
  if (typeof backendReport.value?.planned_duration_seconds === 'number') {
    return backendReport.value.planned_duration_seconds
  }
  return sessionStore.currentSession?.planUsed?.duration || 0
})
...
const improvementText = computed(() =>
  getImprovementText(backendReport.value?.effectiveness, improvement.value, t),
)
...
const initChart = () => {
  ...
  if (backendReport.value?.emotion_curve?.length) {
    labels = backendReport.value.emotion_curve.map(
      (point: any) => point.phase_name || new Date(point.timestamp).toLocaleTimeString(),
    )
    data = backendReport.value.emotion_curve.map((point: any) => point.valence)
  } else {
    labels = sessionStore.emotionHistory.map((_, index) => `${index + 1}`)
    data = sessionStore.emotionHistory.map((emotion) => emotion.valence)
  }
...
onMounted(() => {
  ...
  void (async () => {
    const report = await loadBackendReport()
    if (!report) {
      return
    }
