...
class ReportDataCollector:
    ...
    def collect_from_session(self, session: Session) -> TherapyReport:
        ...
        # 生成情绪变化曲线
        report.emotion_curve = self._generate_emotion_curve(
            session.emotion_history,
            session.final_emotion,
            report.therapy_end_time,
        )
    ...
    def _generate_emotion_curve(
        ...
        # When there is no realtime sampling during therapy, ensure the report
        # still has a terminal point at session end instead of a single dot.
        if final_emotion and end_time:
            last_point = curve_points[-1] if curve_points else None
            if (
                last_point is None
                or last_point.timestamp != end_time
                or last_point.category != final_emotion.category
                or last_point.intensity != final_emotion.intensity
                or last_point.valence != final_emotion.valence
                or last_point.arousal != final_emotion.arousal
            ):
                curve_points.append(
                    EmotionCurvePoint(
                        timestamp=end_time,
                        category=final_emotion.category,
                        intensity=final_emotion.intensity,
                        valence=final_emotion.valence,
                        arousal=final_emotion.arousal,
                        phase_name=last_point.phase_name if last_point else None,
                    )
                )
