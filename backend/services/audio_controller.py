...
def _resolve_audio_file_path(file_path: str) -> Path:
    candidate = Path(file_path)
    if candidate.exists():
        return candidate.resolve()

    project_root_candidate = Path(__file__).resolve().parents[2] / candidate
...
class SoundDeviceAudioController(BaseAudioController):
    ...
    def _load_audio_file(self, file_path: str) -> tuple:
        ...
        Returns:
            (音频数据, 采样率)
        """
        resolved_path = _resolve_audio_file_path(file_path)
        data, sample_rate = sf.read(str(resolved_path), dtype='float32')
...
class TherapyAudioPlayer:
    ...
    async def pause(self) -> bool:
        """Pause any currently playing therapy audio channels."""
        if not self._is_initialized:
            return False

        results = []

        if self._bgm_controller and self._bgm_controller.current_state.is_playing:
            results.append(await self._bgm_controller.pause())
        if self._voice_controller and self._voice_controller.current_state.is_playing:
            results.append(await self._voice_controller.pause())

        return all(results) if results else True

    async def resume(self) -> bool:
        """Resume any paused therapy audio channels."""
        if not self._is_initialized:
            return False

        results = []

        if self._bgm_controller and self._bgm_controller.current_state.state == PlaybackState.PAUSED:
            results.append(await self._bgm_controller.resume())
        if self._voice_controller and self._voice_controller.current_state.state == PlaybackState.PAUSED:
            results.append(await self._voice_controller.resume())
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "bgm": {
                ...
                "loop": self._bgm_controller.current_state.loop if self._bgm_controller else None
            },
            "voice": {
...
_shared_audio_player: Optional[TherapyAudioPlayer] = None


async def get_shared_audio_player() -> TherapyAudioPlayer:
    global _shared_audio_player

    if _shared_audio_player is None:
        player = TherapyAudioPlayer()
        initialized = await player.initialize(use_mock=False)
        if not initialized:
            logger.warning("Falling back to mock therapy audio player")
            initialized = await player.initialize(use_mock=True)

        if not initialized:
            raise RuntimeError("Failed to initialize shared audio player")

        _shared_audio_player = player
