...
class PlanManager:
    ...
    def find_runtime_adjustment_plan(
        ...
        """Find the safest in-session candidate plan for a target intensity."""
        candidates = [
            plan
            for plan in self.plans
            if plan.id != current_plan.id
            and plan.intensity == target_intensity
            and len(plan.phases) == len(current_plan.phases)
        ]

        if not candidates:
            return None

        def sort_key(plan: TherapyPlan) -> tuple[int, int, int, str]:
            style_penalty = 0 if plan.style == current_plan.style else 1
            emotion_penalty = 0
            if emotion is not None:
                emotion_penalty = 0 if emotion.category in plan.target_emotions else 1
            duration_gap = abs(plan.duration - current_plan.duration)
            return (style_penalty, emotion_penalty, duration_gap, plan.name)
