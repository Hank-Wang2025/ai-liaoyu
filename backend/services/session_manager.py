...
class SessionManager:
    ...
    async def mark_stopping(self) -> None:
        """Mark the current session as stopping while teardown completes."""
        if not self._current_session:
            raise ValueError("No active session")

        self._current_session.mark_stopping()
        await self._update_session()

        logger.info(f"Session {self._current_session.id}: Stopping")
        self._notify_listeners("stopping")
