...
def _write_screen_prompt_plan(tmp_path, screen_prompts_block: str = ""):
    plan_path = tmp_path / "screen_prompt_plan.yaml"
    content = textwrap.dedent(
        """
        id: screen_prompt_plan
        name: Screen Prompt Plan
        description: Plan used by screen prompt tests
        target_emotions:
          - anxious
        intensity: medium
        style: modern
...
def _valid_screen_prompts_block():
    return """
screen_prompts:
  - start_second: 0
    end_second: 60
...
class TestTherapyScreenPrompts:
    def test_screen_prompt_plan_loads_valid_timeline(self, tmp_path):
        plan = TherapyPlanParser.load_from_file(
            _write_screen_prompt_plan(tmp_path, _valid_screen_prompts_block())
        )

        assert plan.screen_prompts is not None
        assert len(plan.screen_prompts) == 2
        assert plan.screen_prompts[0].title == "Settle In"
        assert plan.screen_prompts[0].lines == [
            "Breathe in slowly.",
            "Let your shoulders drop.",
        ]
        assert plan.screen_prompts[1].start_second == 60
        assert plan.screen_prompts[1].end_second == 120

    def test_screen_prompt_plan_without_field_defaults_to_none(self, tmp_path):
        plan = TherapyPlanParser.load_from_file(_write_screen_prompt_plan(tmp_path))

        assert plan.screen_prompts is None
        assert plan.id == "screen_prompt_plan"
        assert len(plan.phases) == 1

    @pytest.mark.parametrize(
        ("name", "screen_prompts_block"),
        [
            (
                "first_segment_not_zero",
                """
screen_prompts:
  - start_second: 10
    end_second: 60
    title: Late Start
    lines:
      - Segment starts too late.
  - start_second: 60
    end_second: 120
    title: Finish
    lines:
      - Segment finishes coverage.
""",
            ),
            (
                "overlap",
...
screen_prompts:
  - start_second: 0
    ...
    title: Overlap One
    lines:
      - First overlap segment.
  - start_second: 60
    end_second: 120
    title: Overlap Two
    lines:
      - Second overlap segment.
""",
            ),
            (
                "gap",
...
""",
    ...
    def test_screen_prompt_invalid_group_is_dropped(self, tmp_path, name, screen_prompts_block):
        plan = TherapyPlanParser.load_from_file(
            _write_screen_prompt_plan(tmp_path, screen_prompts_block)
        )

        assert plan.screen_prompts is None, name
        assert plan.id == "screen_prompt_plan"
        assert plan.duration == 120
        assert [phase.name for phase in plan.phases] == ["phase-one"]

    def test_screen_prompt_boundary_matching_and_full_coverage(self, tmp_path):
        plan = TherapyPlanParser.load_from_file(
            _write_screen_prompt_plan(tmp_path, _valid_screen_prompts_block())
        )

        assert plan.screen_prompts[0].start_second == 0
        assert plan.screen_prompts[-1].end_second == plan.duration
        assert plan.screen_prompts[0].end_second == plan.screen_prompts[1].start_second

        matching_zero = [
            prompt
            for prompt in plan.screen_prompts
            if prompt.start_second <= 0 < prompt.end_second
        ]
        matching_last_first_segment = [
            prompt
            for prompt in plan.screen_prompts
            if prompt.start_second <= 59 < prompt.end_second
        ]
        matching_boundary = [
            prompt
            for prompt in plan.screen_prompts
            if prompt.start_second <= 60 < prompt.end_second
        ]

        assert [prompt.title for prompt in matching_zero] == ["Settle In"]
        assert [prompt.title for prompt in matching_last_first_segment] == ["Settle In"]
        assert [prompt.title for prompt in matching_boundary] == ["Stay Here"]

    def test_screen_prompt_round_trip_serialization(self):
        raw_plan = {
            "id": "screen_prompt_plan",
            "name": "Screen Prompt Plan",
            "description": "Plan used by screen prompt tests",
        ...
        plan = TherapyPlan.from_dict(raw_plan)
        serialized = plan.to_dict()

        assert plan.screen_prompts is not None
        assert plan.screen_prompts[0].start_second == 0
        assert plan.screen_prompts[1].title == "Stay Here"
        assert serialized["screen_prompts"] == raw_plan["screen_prompts"]
